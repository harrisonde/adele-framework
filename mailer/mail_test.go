...
func TestMail_SendSMTPMessage(t *testing.T) {
	...
	if err != nil {
		t.Error("Could not send SMTP message.", err)
	}
}
...
func TestMail_SendUsingChan(t *testing.T) {
	...
	msg.To = "bad_email_address"
	mailer.Jobs <- msg
	res = <-mailer.Results
	if res.Error == nil {
		t.Error(errors.New("failed to catch sending with bad email address on channel channel"))
	}
}
...
func TestMail_SendUsingAPI(t *testing.T) {
	...
	mailer.API = "unknown"
	mailer.APIKey = "1234"
	mailer.APIUrl = "https://www.fakemail.com"
